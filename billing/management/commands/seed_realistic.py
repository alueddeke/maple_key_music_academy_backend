...
Creates:
  ...
  - July 2026: draft batches (teachers submit in E2E Step 1, no student invoices yet)
  - August 2026: approved-but-not-invoiced batches (Phase 22 month-end queue — no teacher
    invoice yet, batch.invoice IS NULL, teachers mark exceptions before management generates)
...
def _approve_batch_no_invoice(batch, school, items_by_student, mgmt):
    """Phase 22: approve batch without creating teacher invoice — batch.invoice stays NULL.

    Batch appears in month-end queue (management/batches/month-end-queue/).
    No waived_rollover CreditTransaction written — that only happens at generate time.
    """
    student_invoices = []
    ...
    with transaction.atomic():
        ...
        # batch.invoice intentionally left NULL — teacher invoice generated by management later
        batch.status = 'approved'
        batch.reviewed_by = mgmt
        batch.reviewed_at = timezone.now()
        batch.submitted_at = timezone.now()
...
class Command(BaseCommand):
    ...
    def _seed_approved_no_invoice_period(self, school, teachers, all_records, month, year, mgmt):
        """Phase 22: seed approved-but-not-invoiced batches — appear in month-end queue."""
        import calendar as _cal
        self.stdout.write(self.style.WARNING(
            f'\nSeeding {_cal.month_name[month]} {year} (approved — awaiting teacher invoice)…'
        ))
        ...
        for teacher in teachers:
            ...
            # Phase 22: approved-but-not-invoiced batch for month-end queue
            batch.lesson_items.update(status='completed')
            student_invoices = _approve_batch_no_invoice(batch, school, items_by_student, mgmt)
            total_invoiced += len(student_invoices)

            self.stdout.write(
                f'  ✓ {teacher.get_full_name()} — batch {batch.batch_number} '
                f'(approved, invoice=NULL, {len(student_invoices)} student invoices)'
            )

        self.stdout.write(
            f'  → {len(teachers)} batch(es) in month-end queue | {total_invoiced} student invoices'
        )
    ...
    def _print_preview(self):
        ...
        self.stdout.write(f'Periods  : May 2026 (approved+paid), June 2026 (approved+paid), July 2026 (draft), August 2026 (approved, month-end queue)')
    ...
    def _print_summary(self, all_records):
        ...
        self.stdout.write(self.style.SUCCESS('━' * 64))
        from billing.models import MonthlyInvoiceBatch as _Batch
        queue_count = _Batch.objects.filter(status='approved', invoice__isnull=True).count()
        self.stdout.write(f'  Periods : May 2026 ✓ paid | June 2026 ✓ mixed | July 2026 📋 draft | August 2026 ⏳ month-end queue')
        self.stdout.write(f'  Month-end queue: {queue_count} approved batch(es) awaiting teacher invoice')
