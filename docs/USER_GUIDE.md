...
```mermaid
flowchart TD
    A[Management sends invitation] --> B[User receives email with 48-hour link]
    B --> C[User clicks link]
    C --> D{Token valid?}
    D -->|Yes| E[Account setup page]
    D -->|No| F[Error: Invalid or expired token]
    E --> G{Choose login method}
    G -->|Email/Password| H[Enter name + password]
    G -->|Google OAuth| I[Click 'Sign In with Google']
    G -->|Both| J[Enter name + password + link Google]
    H --> K[Account created]
    I --> K
    J --> K
    K --> L[Auto-login to dashboard]
```
...
sequenceDiagram
    actor Teacher
    participant UI as Monthly Invoices Page
    participant API as Backend API
    participant DB as Database

    Teacher->>UI: Select month/year
    UI->>API: GET/POST batch for period
    API->>DB: Get or create MonthlyInvoiceBatch
    DB->>API: Return batch with auto-populated lessons
    API->>UI: Display batch in draft status

    Teacher->>UI: Update lesson statuses
    Teacher->>UI: Add one-off lessons
    UI->>API: PATCH lesson items
    API->>DB: Update lesson items

    Teacher->>UI: Click "Submit to Management"
    UI->>API: POST submit batch
    API->>DB: Update batch.status = 'submitted'
    DB->>API: Confirmation
    API->>UI: Success message
    UI->>Teacher: "Batch submitted for review"
```
...
sequenceDiagram
    actor Management
    participant UI as Batch Detail Page
    participant API as Backend API
    participant DB as Database

    Management->>UI: Navigate to pending batch
    UI->>API: GET batch details
    API->>DB: Fetch batch with lesson items
    API->>API: Run validation checks
    DB->>API: Return batch data
    API->>UI: Display batch + validation results

    Management->>UI: Review lesson items
    Management->>UI: Click "Approve & Export CSV"
    UI->>API: POST approve batch

    API->>DB: Begin transaction
    DB->>DB: Create StudentInvoice records
    DB->>DB: Update batch.status = 'approved'
    DB->>API: Commit transaction

    API->>API: Generate Helcim CSV
    API->>UI: Return CSV file
    UI->>Management: Download CSV + success message
```
...
stateDiagram-v2
    ...
    submitted --> draft: Management rejects
    approved --> [*]

    note right of draft: Teacher can edit
    note right of submitted: Read-only, under review
    note right of approved: Paystub available
```
...
```mermaid
flowchart LR
    A[Batch submitted] --> B[Management reviews]
    B --> C{Approval decision}
    C -->|Approved| D[Status: approved]
    C -->|Rejected| E[Status: draft]
    E --> F[Teacher sees rejection reason]
    F --> G[Teacher makes corrections]
    G --> H[Teacher resubmits]
    H --> A
```
...
```mermaid
flowchart TB
    A[Management creates recurring schedule] --> B[Schedule defines weekly lessons]
    B --> C[Teacher creates/views monthly batch]
    C --> D[System auto-populates lessons from schedules]
    D --> E[Teacher adds one-off lessons]
    E --> F[Teacher marks statuses: completed/cancelled]
    F --> G[Teacher submits batch]
    G --> H[Management reviews batch]
    H --> I{Validation passes?}
    I -->|No| J[Identify missing billing info]
    J --> K[Update student billable contacts]
    K --> H
    I -->|Yes| L{Approve or reject?}
    L -->|Reject| M[Enter rejection reason]
    M --> N[Batch returns to draft]
    N --> F
    L -->|Approve| O[Create StudentInvoice records]
    O --> P[Generate Helcim CSV]
    P --> Q[Teacher paystub available]
    Q --> R[Upload CSV to payment system]
```
...
```mermaid
flowchart TB
    A[New User Wants Account] --> B{Which path?}

    B -->|Path 1: Pre-Approval| C[Management adds email to approved list]
    C --> D[System sends 48-hour invitation link]
    D --> E[User sets up account]
    E --> F[Immediate access]

    B -->|Path 2: Open Registration| G[User submits registration request]
    G --> H[Request appears in pending list]
    H --> I[Management reviews]
    I --> J{Approve or reject?}
    J -->|Approve| K[System sends invitation link]
    K --> E
    J -->|Reject| L[Request deleted]

    B -->|Path 3: OAuth| M[User attempts Google login]
    M --> N{Email pre-approved?}
    N -->|Yes| F
    N -->|No| H
```
